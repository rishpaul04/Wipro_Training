...
class LoginPage(BasePage):
    ...
    def click_login_button(self):
        """Click the login button and wait for the result"""
        self.click(*self.LOGIN_BUTTON)
        # Wait for either navigation to inventory or an error message
    ...
    def _error_text(self):
        """Read the error container text if it contains an error"""
        try:
            element = self.driver.find_element(*self.ERROR_MESSAGE)
            text = element.text
            return text if element.is_displayed() else ""
        except Exception:
            return ""
